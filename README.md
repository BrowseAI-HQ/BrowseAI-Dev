...
2. **Domain authority scoring** — 150+ domains across 5 tiers (institutional `.gov`/`.edu` → major news → tech journalism → community → low-quality). Scores self-improve over time using Bayesian cold-start smoothing — every query feeds back verification data to make future scores more accurate.
